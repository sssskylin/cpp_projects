...
TEST(TransformFilterTest, OddPowerOfTwo) {
    std::vector<int> input = {1, 2, 3, 4, 5};
    auto result = AsDataFlow(input) | Transform(PowerOfTwo) | Filter([](int x) { return x % 2 != 0; }) | AsVector();
    ASSERT_THAT(result, testing::ElementsAre(1, 9, 25));
}
